...
descriptions_csv <- read.csv('data/comm_wealth_vars.csv')

descriptions <- list()
for (var in vars) {
  my_desc <- descriptions_csv$Description[match(var, descriptions_csv$Variable)]
...
sources <- list()
for (var in vars) {
  my_source <- descriptions_csv$Source[match(var, descriptions_csv$Variable)]
...
gt_tbl <- d |>
  ...
  tab_row_group(
    label = html("<strong>Uncategorized</strong>"),
...
Data on county-level poverty rates were obtained from the 2022 US Census.
...
Data on total agricultural land were obtained from the 2022 Census of Agriculture.
...
County-level classifications as "rural" and/or "underserved were obtained from the US Consumer Finance Bureau...
...
The categorization of each county on the rural-urban continuum came from the USDA-ERS...
