...
farm_size <- farms %>%
  ...
  mutate(mean_farm_acres = acres / farms)
```

### Rural vs Urban data from census?
...
all_data <- comm_wealth_wide %>%
  ...
  mutate(
    ...
    award_per_capita = normalize(award_per_capita),
    is_rural = ifelse(fips %in% rural_data$FIPS.Code, 1, 0),
    is_underserved = ifelse(fips %in% underserved_data$FIPS.Code, 1, 0)
    ) %>%
  merge(farm_size, by.x = 'fips', by.y = 'GEOID') %>%
  merge(urban_rural %>% select(GEOID, rural_pct), by.x = 'fips', by.y = 'GEOID')
```
...
data$is_rural <- as.factor(data$is_rural)
data$is_underserved <- as.factor(data$is_underserved)
