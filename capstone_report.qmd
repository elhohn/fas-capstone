...
p <- ggplot(clustered_melted, aes(cluster, value), shape = 21) +
  geom_jitter(
    aes(color = factor(lamp_funded),
        fill = factor(cluster)),
    shape=21, size = 1, alpha = 0.5, stroke = 0.9) +
  scale_fill_manual(values = as.vector(paletteer_d("rcartocolor::Prism"))[1:length(unique(clustered_melted$cluster))]) +
  scale_colour_manual(
