...
lamp_by_county <- counties %>%
  ...
  select(GEOID, County, lamp_funded, Award.Amount)
```
...
leaflet(pov_sf) %>%
  ...
  addLegend("bottomright", pal = pov_pal, values = ~poverty_rate,
    title = "Poverty Rate",
    #labFormat = labelFormat(prefix = "$"),
    opacity = 1
  )
...
ag_proportion <- ag_acres %>%
  select(GEOID, Value, county_acres) %>%
  drop_na() %>%
  rename(ag_acres = Value) %>%
  mutate(ag_proportion = ag_acres / county_acres) %>%
  #st_drop_geometry() %>%
  select(GEOID, ag_proportion) %>%
  drop_units() %>%
  st_transform(4326)

ag_pal <- colorBin(palette = c("#FAE9A0FF", "#DBD797FF", "#BCC68DFF", "#9CB484FF", "#7DA37BFF", "#5E9171FF", "#3F7F68FF", "#1F6E5EFF", "#005C55FF"),#paletteer_d("Redmonder::sPBIYlGn"), 
                   domain = ag_proportion$ag_proportion,
                   bins = 9)

labels <- sprintf(
  "<strong>%s</strong><br/>Percentage in ag: %s percent",
  ag_proportion$GEOID, round(ag_proportion$ag_proportion*100, 1)
) %>% lapply(htmltools::HTML)
...
leaflet(ag_proportion) %>%
  ...
  addLegend("bottomright", pal = ag_pal, values = ~ag_proportion,
...
pca_reduce <- function(d){
  ...
  if (comps > 0) {
    scores <- pca_results$ind$coord %>% as_tibble()
