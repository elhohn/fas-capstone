...
### Load county to zip code crosswalk data from HUD

Dataset was downloaded from HUD [site](https://www.huduser.gov/portal/datasets/usps_crosswalk.html "HUD site"). Note: you must be registered and logged in to access data. Note: page 6 of [this document](https://www.huduser.gov/portal/periodicals/cityscpe/vol20num2/ch16.pdf "HUD data explainer") explains the data.

`res_ratio` column is the ratio of residential addresses.

`bus_ratio` column is the ratio of business addresses

`oth_ratio` column is the ratio of other addresses to the total number of other addresses in the entire ZIP.

`tot_ratio` column is the total ratio of all addresses to the total number of addresses in the entire ZIP.
