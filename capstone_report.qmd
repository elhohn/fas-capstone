...
normalize <- function(x){
  round((x - min(x, na.rm = TRUE)) / (max(x, na.rm = TRUE) - min(x, na.rm = TRUE)), 2)
}
...
leaflet() %>%
  ...
  addPolygons(
    ...
    group = "Population") %>%
  addLayersControl(
    baseGroups = c("Infrastructure", "Food Insecurity", "Population"),
    #overlayGroups = c("Infrastructure", "Outline"),
