...
comm_wealth_wide[cols_to_normalize] <- apply(comm_wealth_wide[cols_to_normalize], 2, function(x) (x - min(x, na.rm = TRUE)) / (max(x, na.rm = TRUE) - min(x, na.rm = TRUE)))
...
all_data <- comm_wealth_wide %>%
  merge(lamp_county_summary, by.x = 'fips', by.y = 'GEOID', all.x = TRUE) %>%
  st_drop_geometry() %>%
