...
h <- infrastructure %>%
  ...
  theme(
    ...
    strip.text = element_text(colour = 'grey76'),
...
#ggarrange(c, h, ncol = 1, nrow = 2)
ggplotly(h) %>% config(displayModeBar = FALSE)

#pca_results <- PCA(pca_prep(d), graph = FALSE)
pca_results <- prcomp(infrastructure, scale = TRUE)
...
## Community Characteristics" "Processing & Distribution" "Food Access"
